...
# ST-ResNet网络
def stresnet_multi_step_pre(c_conf=(len_seq3, N_flow, N_station), p_conf=(len_seq2, N_flow, N_station),
                            ...
                            external_dim7=None, external_dim8=None, external_dim9=None, nb_residual_unit=3,
                            nb_edge_residual_unit=3, pre_step=1):
    # main input
    main_inputs = []
    main_outputs = []
    outputs = []
    ...
    for conf in [c_conf, p_conf, t_conf]:
        if conf is not None:
            ...
            # input00 = Input(shape=(stations * pre_step, nb_feature * len_seq))
            # Conv1
            conv1 = Convolution1D(
    ...
    for conf1 in [c1_conf, p1_conf, t1_conf]:
        ...
        if conf1 is not None:
            ...
            # input11 = Input(shape=(stations * pre_step, nb_feature * len_seq))

            # Conv1
            conv11 = Convolution1D(nb_filter=64, filter_length=5, border_mode="same")(input1)
            # [nb_residual_unit] Residual Units
            ...
            conv31_1 = Convolution1D(
                nb_filter=stations, filter_length=1, border_mode="same")(conv21)
            conv31_2 = Convolution1D(
                nb_filter=stations, filter_length=3, border_mode="same")(conv21)
            conv31_3 = Convolution1D(
                nb_filter=stations, filter_length=5, border_mode="same")(conv21)
    ...
    conv_node0 = Convolution1D(nb_filter=nb_flow * pre_step, filter_length=3, border_mode="same")(main_output)
    # conv_node0 = Dense(int(conv_node0.shape[1]) * pre_step * int(conv_node0.shape[2]))(conv_node0)
    # 重新reshape一下
    # conv_node1 = tf.reshape(conv_node0, shape=[-1, int(conv_node0.shape[1]) * pre_step, int(conv_node0.shape[2])])
    conv_node1 = Lambda(lambda x: tf.reshape(x, (-1, int(conv_node0.shape[1]) * pre_step, nb_flow)))(
        conv_node0)
    conv_node = Convolution1D(nb_filter=nb_flow, filter_length=3, border_mode="same")(conv_node1)

    conv_edge0 = Convolution1D(nb_filter=nb_stations * pre_step, filter_length=3, border_mode="same")(main_output)
    # conv_edge0 = Dense(int(conv_edge0.shape[1]) * pre_step * int(conv_edge0.shape[2]))(conv_edge0)
    # conv_edge1 = tf.reshape(conv_edge0, shape=[-1, int(conv_edge0.shape[1]) * pre_step, int(conv_edge0.shape[2])])
    conv_edge1 = Lambda(lambda x: tf.reshape(x, (-1, int(conv_edge0.shape[1]) * pre_step, nb_stations)))(conv_edge0)
    conv_edge = Convolution1D(nb_filter=nb_stations, filter_length=3, border_mode="same")(conv_edge1)
    ...
    if external_dim1 is not None and external_dim1 > 0:
        ...
        # node 外部信息加入到node数据中
        h1 = Dense(output_dim=nb_flow * nb_stations * pre_step)(embedding1)
        activation4 = Activation('relu')(h1)
        external_output1 = Reshape((nb_stations * pre_step, nb_flow))(activation4)
        main_output1 = Add()([conv_node, external_output1])
        main_outputs[0] = main_output1
        # edge 外部信息加入到edge数据中
        h11 = Dense(output_dim=nb_stations * nb_stations * pre_step)(embedding1)
        activation5 = Activation('relu')(h11)
        external_output11 = Reshape((nb_stations * pre_step, nb_stations))(activation5)
    ...
    if external_dim2 is not None and external_dim2 > 0:
        ...
        # node 外部信息加入到node数据中
        h1_2 = Dense(output_dim=nb_flow * nb_stations * pre_step)(embedding2)
        activation4_2 = Activation('relu')(h1_2)
        external_output1_2 = Reshape((nb_stations * pre_step, nb_flow))(activation4_2)
        main_output1_2 = Add()([conv_node, external_output1_2])
        main_outputs[0] = main_output1_2
        # edge 外部信息加入到edge数据中
        h11_2 = Dense(output_dim=nb_stations * nb_stations * pre_step)(embedding2)
        activation5_2 = Activation('relu')(h11_2)
        external_output11_2 = Reshape((nb_stations * pre_step, nb_stations))(activation5_2)
    ...
    if external_dim4 is not None and external_dim4 > 0:
        ...
        # node 外部信息加入到node数据中
        h1_4 = Dense(output_dim=nb_flow * nb_stations * pre_step)(embedding4)
        activation4_4 = Activation('relu')(h1_4)
        external_output1_4 = Reshape((nb_stations * pre_step, nb_flow))(activation4_4)
        main_output1_4 = Add()([conv_node, external_output1_4])
        main_outputs[0] = main_output1_4
        # edge 外部信息加入到edge数据中
        h11_4 = Dense(output_dim=nb_stations * nb_stations * pre_step)(embedding4)
        activation5_4 = Activation('relu')(h11_4)
        external_output11_4 = Reshape((nb_stations * pre_step, nb_stations))(activation5_4)
    ...
    if external_dim5 is not None and external_dim5 > 0:
        ...
        # node 外部信息加入到node数据中
        h1_5 = Dense(output_dim=nb_flow * nb_stations * pre_step)(embedding5)
        activation4_5 = Activation('relu')(h1_5)
        external_output1_5 = Reshape((nb_stations * pre_step, nb_flow))(activation4_5)
        main_output1_5 = Add()([conv_node, external_output1_5])
        main_outputs[0] = main_output1_5
        # edge 外部信息加入到edge数据中
        h11_5 = Dense(output_dim=nb_stations * nb_stations * pre_step)(embedding5)
        activation5_5 = Activation('relu')(h11_5)
        external_output11_5 = Reshape((nb_stations * pre_step, nb_stations))(activation5_5)
    ...
    if external_dim9 is not None and external_dim9 > 0:
        ...
        # node 外部信息加入到node数据中
        h1_9 = Dense(output_dim=nb_flow * nb_stations * pre_step)(embedding9)
        activation4_9 = Activation('relu')(h1_9)
        external_output1_9 = Reshape((nb_stations * pre_step, nb_flow))(activation4_9)
        main_output1_9 = Add(name='node_logits')([conv_node, external_output1_9])  # 改了这里
        main_outputs[0] = main_output1_9
        # edge 外部信息加入到edge数据中
        h11_9 = Dense(output_dim=nb_stations * nb_stations * pre_step)(embedding9)
        activation5_9 = Activation('relu')(h11_9)
        external_output11_9 = Reshape((nb_stations * pre_step, nb_stations))(activation5_9)
        main_output2_9 = Add(name="edge_logits")([conv_edge, external_output11_9])
