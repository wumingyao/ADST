...
def train_Arima(data):
    ...
    np.save('./npy/mae_compare/predict_arima_day25.npy', predict_arima)
...
def train_stresnet(Metro_Flow_Matrix, Metro_Edge_Flow_Matrix):
    ...
    model = stresnet(c_conf=(len_seq3, N_flow, N_station), p_conf=(len_seq2, N_flow, N_station),
                     ...
                     t1_conf=(len_seq1, N_station, N_station),  # 边流量input
                     external_dim1=1, external_dim2=1, external_dim3=None,  # 外部信息
                     external_dim4=1, external_dim5=81, external_dim6=None,
                     external_dim7=None, external_dim8=None, external_dim9=1,
                     nb_residual_unit=4, nb_edge_residual_unit=4)  # 这里的unit代表了大体的网络深度
    ...
    # 输入的东西要注意
    history = model.fit([xr_train, xp_train, xt_train, xredge_train, xpedge_train, xtedge_train, mask_matrix,
                        ...
                        [target, edge_target],
                        validation_data=([xr_val, xp_val, xt_val, xredge_val, xpedge_val, xtedge_val, val_mask_matrix,
...
def train_LSTM(Metro_Flow_Matrix, Metro_Edge_Flow_Matrix):
    warnings.filterwarnings('ignore')
    ...
    model = LSTM_Net(c_conf=(len_seq3, N_flow, N_station), p_conf=(len_seq2, N_flow, N_station),
                     ...
                     external_dim4=1, external_dim5=81, external_dim6=None,
                     external_dim7=None, external_dim8=None, external_dim9=1,
                     nb_residual_unit=12, nb_edge_residual_unit=12)  # 这里的unit代表了大体的网络深度
    model.compile(optimizer='adam',
                  loss={'node_logits': my_own_loss_function, 'edge_logits': my_own_loss_function},
                  loss_weights={'node_logits': 1, 'edge_logits': 0.2},  # 两个任务各自的权重
                  metrics=['accuracy'])

    filepath = config.filepath_LSTM
    # 中途训练效果提升, 则将文件保存, 每提升一次, 保存一次
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=0, save_best_only=True, mode='min')
    reduce_lr = LearningRateScheduler(scheduler)
    early_stopping = EarlyStopping(monitor='val_loss', patience=30, verbose=0, mode='min')
